...
def write_trajectory_file(x_list, y_list, v_list, t_list, s_list, th_list, vcmd_list):
    create_controller_output_dir(CONTROLLER_OUTPUT_FOLDER)
    file_name = os.path.join(CONTROLLER_OUTPUT_FOLDER, 'trajectory.txt')

    with open(file_name, 'w') as trajectory_file: 
        for i in range(len(x_list)):
            trajectory_file.write('%3.3f, %3.3f, %2.3f, %6.3f, %6.3f, %6.3f, %2.3f\n' %\
                                  (x_list[i], y_list[i], v_list[i], t_list[i], s_list[i], th_list[i], vcmd_list[i]))
...
def exec_waypoint_nav_demo(args):
    ...
    with make_carla_client(args.host, args.port) as client:
        ...
        for frame in range(TOTAL_EPISODE_FRAMES):
            ...
            ###
            # Controller update (this uses the controller2d_PurePursuit.py implementation)
            ###
            ...
            # append steering command
            steer_history.append(cmd_steer)
            throttle_history.append(cmd_throttle)
            vcmd_history.append(controller._desired_speed)
        ...
        write_trajectory_file(x_history, y_history, speed_history, time_history, steer_history, throttle_history, vcmd_history)
